...
1. `data/krx-history/` 파일 절대 수정/삭제 금지 (243MB 로컬 전용 데이터)
2. `scripts/*.mjs` 파일 절대 삭제 금지 (untracked, 복구 불가)
3. `git push --force` 절대 금지
4. `git clean -fd` 실행 전 반드시 사용자 확인 받기
5. Redis 키 직접 삭제 전 반드시 사용자 확인 받기
6. 환경변수 값을 코드에 하드코딩 금지

---

## 📋 작업 시작 전 계획서 작성

코드를 작성하기 전에 아래 형식의 계획서를 먼저 작성한다.

**계획서 형식:**
1. **작업 목표**: 무엇을 달성하려는가
2. **구현 단계**: 순서대로 어떤 작업을 할 것인가
3. **변경될 파일 목록**: 어떤 파일을 생성/수정/삭제하는가
4. **완료 기준**: 어떤 상태가 되면 완료인가
5. **예상 위험 요소**: 무엇이 잘못될 수 있는가
6. **실수할 수 있는 부분**: 특히 주의할 점은 무엇인가

**규칙:**
- 관련 파일을 전체 읽기 전에 코드 작성 금지
- 확실하지 않으면 추측하지 말고 질문할 것
- 빠른 완료보다 정확한 완료를 우선할 것
- 사용자 OK 전에 코드 작성 금지
- 예상과 다른 상황이 생기면 멈추고 보고할 것
- 각 단계 완료 후 보고할 것

---

## 🗒️ 작업 상태 파일 (CURRENT_TASK.md)

작업 시작 시 프로젝트 루트에 `CURRENT_TASK.md` 파일을 생성하고 유지한다.
새 대화 창에서 작업을 이어받을 때 반드시 이 파일을 먼저 읽는다.
...
코드 수정이 끝나면 아래를 순서대로 실행하고 결과를 확인한다.

1. `npx tsc --noEmit`              # 타입 오류 확인
2. `npm run lint`                  # 린트 오류 확인
3. `npm run build`                 # 빌드 성공 여부 확인
4. `node scripts/review-agent.mjs` # 리뷰어 클로드가 독립적으로 코드 검토 → review-result.md 확인

하나라도 오류가 나면 수정 후 다시 확인. "완료"라고 말하기 전에 반드시 통과해야 함.

---

## 🧹 Lint 정리 작업 가이드

여러 lint 에러가 누적된 상태를 정리할 때 아래 패턴을 따른다.

### 1. ESLint `react-hooks/set-state-in-effect` 룰의 특성

이 룰은 **한 useEffect 안의 첫 번째 setState만 보고**한다. 첫 번째를 fix하면 다음 setState가 새로 노출된다.

```ts
// 진단 시점: setUnmount(true) 1건만 잡힘
useEffect(() => {
  if (sessionStorage.getItem("x")) {
    setUnmount(true);  // ❌ 잡힘
    return;
  }
  setShow(true);       // (안 잡힘 - 첫 에러에 가려짐)
  setPageLoaded(true); // (안 잡힘)
}, []);
```

→ **진단 보고서의 에러 N건 ≠ 최종 수정 N건.** 매 수정 후 격리 lint를 즉시 재확인:
...
### 2. set-state-in-effect 대응 fix 패턴 (우선순위 순)

| 우선순위 | 패턴 | 예시 |
|---|---|---|
| 1 | **derived state로 변환** | `const fadeOut = a && b` (별도 state 제거) |
| 2 | **useState lazy initializer** | `useState(() => Math.random() * N)` |
| 3 | **`requestAnimationFrame` 감싸기** | `requestAnimationFrame(() => setShow(true))` |
| 4 | **이벤트 기반으로 전환** (useEffect 자체 제거) | `pathname` 변경 추적 → `Link onClick` 핸들러 |
| 5 | **`useSyncExternalStore`** | 외부 상태/sessionStorage 읽기 |

위에서부터 시도. 가능하면 1번이 가장 안전 (state 자체를 줄임).

### 3. 권장 워크플로우

**1단계 — 진단 (수정 X)**
```bash
npx eslint app/ components/ lib/ hooks/ -f json --output-file lint-report.json
```
- 룰별/위험도별로 카테고리화
- 임시 진단 파일(`lint-report.json` 등)은 작업 끝나면 정리

**2단계 — 배치 분리**
- 낮음/중간 위험도 → 한 커밋
- UX 민감 부분(splash screen, navbar 등) → 별도 커밋
- 사이트 전반 영향 큰 것은 마지막에

**3단계 — 각 파일 수정 후 격리 lint**
```bash
npx eslint <file>
```
- 새 에러 노출 시(룰 first-error 특성) 같은 사이클 반복
...
**5단계 — 푸시 전 보고**
- UX 영향 있는 항목은 변경 전후 동작 차이를 명시해 보고
- 시각적 회귀가 의심되면 `npm run dev`로 직접 확인 권장
