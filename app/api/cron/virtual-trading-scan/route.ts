...
export async function GET() {
  ...
  console.log(`[virtual-trading-scan] 스캔 시작: ${todayDate}`);
  ...
  for (const pos of state.positions) {
    const current = stockMap.get(pos.code);
    if (current && current.closePrice > pos.highestHigh) {
      pos.highestHigh = current.closePrice;
      pos.trailingStopPrice = Math.round(current.closePrice * 0.97);
    }
  }

  // 주식 평가액 갱신 → equity curve
  ...
  return NextResponse.json({
    ...
    console.error("[virtual-trading-scan] 스캔 중 에러:", error);
    return NextResponse.json(
