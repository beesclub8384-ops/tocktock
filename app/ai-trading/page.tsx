    ...
    >
      <div data-draggable-modal className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-2xl" style={{ transform: `translate(${position.x}px, ${position.y}px)`, ...(size.width ? { width: size.width, height: size.height } : { width: "100%", maxWidth: "48rem" }) }}>
        <div className="overflow-y-auto p-6 sm:p-8" style={{ maxHeight: size.height ? size.height - 2 : "85vh" }}>
          ...
          <h2 className="mb-6 text-xl font-bold cursor-move select-none" onMouseDown={handleMouseDown}>AI 자동매매 시뮬레이터 안내</h2>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            <section>
              ...
              <ul className="space-y-1.5 text-sm leading-relaxed text-muted-foreground">
                <li>
                  <strong className="text-foreground">목표가:</strong> 매수가 대비 +5% 도달 시 즉시 매도
                </li>
                <li>
                  <strong className="text-foreground">손절가:</strong> 매수가 대비 -3% 도달 시 즉시 매도
                </li>
                <li>매수 즉시 지정가 매도 + 스탑로스 설정</li>
              </ul>
            </section>
          </div>
...
export default function AiTradingPage() {
  ...
  const handleReset = async () => {
    ...
    setResetting(true);
    try {
      await fetch("/api/ai-trading", { method: "DELETE" });
      fetchData();
    } catch {
      alert("초기화 실패");
    } finally {
      ...
      <div className="mx-auto max-w-5xl px-6 py-12">
        ...
        <header className="mb-8">
          ...
          <p className="text-sm text-muted-foreground">
            가상 자금 1,000만원으로 세력진입 패턴 기반 AI 자동매매를 시뮬레이션합니다 (+5% 목표가 / -3% 손절)
          </p>
        ...
        {activeTab === "positions" && (
          <div className="space-y-3">
            {positions.length === 0 ? (
              ...
              positions.map((p) => {
                const targetPnlRate =
                  ((p.targetPrice - p.buyPrice) / p.buyPrice) * 100;
                const stopPnlRate =
                  ((p.stopLossPrice - p.buyPrice) / p.buyPrice) * 100;
                return (
                  <div
                    key={p.code}
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-xs text-muted-foreground">
                          목표가 (+{targetPnlRate.toFixed(0)}%)
                        </span>
                        <div
                          className="font-medium text-red-400"
                          style={{ fontFamily: "'DM Mono', monospace" }}
                        >
                          {p.targetPrice.toLocaleString()}원
                        </div>
                      </div>
                      <div>
                        <span className="text-xs text-muted-foreground">
                          손절가 ({stopPnlRate.toFixed(0)}%)
                        </span>
                        <div
                          className="font-medium text-blue-400"
                          style={{ fontFamily: "'DM Mono', monospace" }}
                        >
                          {p.stopLossPrice.toLocaleString()}원
                        </div>
                      </div>
                    </div>
                  </div>
