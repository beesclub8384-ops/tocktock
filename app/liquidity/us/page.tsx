...
const GUIDE_DATA: Record<string, { title: string; body: string }> = {
  "net-liquidity": {
    title: "연준 순유동성이란?",
    body: "연준의 총자산(WALCL)에서 역레포(RRPONTSYD)와 재무부 일반계좌(TGA)를 뺀 값입니다. 시중에 실제로 돌아다니는 돈의 양을 나타냅니다. 이 값이 늘면 주식시장에 유리하고, 줄면 불리합니다. 나스닥과 1~3개월 시차를 두고 높은 상관관계를 보입니다.",
  },
  "m2-growth": {
    title: "M2 통화량 증가율이란?",
  ...
  "hy-spread": {
    title: "하이일드 스프레드란?",
    body: "신용등급이 낮은 기업(정크본드)이 미국 국채보다 얼마나 더 높은 이자를 내야 하는지 나타냅니다. 스프레드가 좁으면 투자자들이 위험을 감수할 의지가 높다는 뜻으로 유동성이 풍부한 상태입니다. 넓어지면 크레딧 시장이 경색되고 있다는 경고입니다.",
  },
  "ig-spread": {
    title: "IG 크레딧 스프레드란?",
    body: "투자등급(BBB 이상) 회사채와 국채의 금리 차이입니다. 하이일드보다 안전한 기업의 자금조달 비용을 나타내며, 이것마저 벌어지면 전반적인 기업 자금조달 환경이 나빠지고 있다는 강한 신호입니다.",
  },
  nfci: {
    title: "NFCI (금융상황지수)란?",
    body: "시카고 연준이 매주 발표하는 종합 금융상황 지표입니다. 105개의 금융 변수(금리, 스프레드, 레버리지 등)를 종합합니다. 0 이하이면 금융환경이 완화적(유동성 풍부), 양수이면 긴축적(유동성 부족)입니다.",
  },
  "vix-3m": {
    title: "VIX 3개월 평균이란?",
    body: "S&P500 옵션에서 추출한 향후 30일 변동성 기대치(VIX)의 63거래일 평균입니다. 단기 급등락을 제거하고 추세를 봅니다. VIX가 낮으면 시장이 안정적이고 유동성이 원활하다는 뜻이며, 높으면 불안과 유동성 경색 신호입니다.",
  },
};
    ...
    <main className="max-w-4xl mx-auto px-4 py-12 sm:px-8 sm:py-20">
      <header className="mb-10">
        ...
        <p className="mt-2 text-muted-foreground">
          연준 유동성·크레딧·시장 상황을 종합한 나스닥 1~3개월 선행 지표
        </p>
        {data?.fetchedAt && (
